...
class ParametreController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            $parametre = Parametre::with('media')->first();
            ...
            $mediaFields = ['cover', 'logo_header', 'logo_footer'];
            foreach ($mediaFields as $field) {
                if ($request->hasFile($field)) {
                    // Supprime l'ancien média s’il existe
                    if ($parametre->hasMedia($field)) {
                        $parametre->clearMediaCollection($field);
                    }

                    // Ajoute le nouveau fichier
                    $parametre->addMediaFromRequest($field)->toMediaCollection($field);
                }
            }

            Alert::success('Opération réussie', 'Les paramètres ont été enregistrés avec succès.');
            return back();
        } catch (\Throwable $th) {
            return back()->withError($th->getMessage());
        }
